...
class Agent():
    def __init__(self, alpha, beta, obs_dim, tau, n_actions, n_agents, gamma=0.95,
        ...
        self.target_critic = CriticNetwork(beta, obs_dim, fc1_dims, fc2_dims,
    ...
    def learn(self):
        ...
        obs_c = obs.view(self.batch_size*self.n_agents, self.obs_dim).to(self.actor.device)
        obs_c_ = obs_.view(self.batch_size*self.n_agents, self.obs_dim).to(self.actor.device)
        ...
        target_actions = self.target_actor.forward(obs_c_)
        ...
        qs = self.critic.forward(obs_c, sample_actions)
        qs = agents_qs.view(self.batch_size, self.n_agents, self.q_dim)
        m_qs =  self.qmixer.forward(qs, state)
